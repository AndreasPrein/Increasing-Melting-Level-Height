...
StartTime=datetime.datetime(1979, 01, 01)  #Y-M-D --> days are smallest possible period
...
for st in range(len(rgsStatID)):
    ...
    if os.path.isfile(SSaveName+'.npz') == 0:
        ...
        rgiSoundingsAct=np.array(range(len(content)))[rgbSoundingsAct]
...
rgrSnowProb=np.nanmean(np.reshape(rgrSnowProb, (rgrSnowProb.shape[0],rgrSnowProb.shape[1]/2,2)), axis=2)
rgrWBZ=np.nanmean(np.reshape(rgrWBZ, (rgrWBZ.shape[0],rgrWBZ.shape[1]/2,2)), axis=2)
rgr3CLevel=np.nanmean(np.reshape(rgr3CLevel, (rgr3CLevel.shape[0],rgr3CLevel.shape[1]/2,2)), axis=2)
rgrFreezingLevel=np.nanmean(np.reshape(rgrFreezingLevel, (rgrFreezingLevel.shape[0],rgrFreezingLevel.shape[1]/2,2)), axis=2)
rgrTS=np.nanmean(np.reshape(rgrTS, (rgrTS.shape[0],rgrTS.shape[1]/2,2)), axis=2)
rgrLRbF=np.nanmean(np.reshape(rgrLRbF, (rgrLRbF.shape[0],rgrLRbF.shape[1]/2,2)), axis=2)
rgrCAPE=np.nanmean(np.reshape(rgrCAPE, (rgrCAPE.shape[0],rgrCAPE.shape[1]/2,2)), axis=2)
rgrCIN=np.nanmean(np.reshape(rgrCIN, (rgrCIN.shape[0],rgrCIN.shape[1]/2,2)), axis=2)
rgrLCL=np.nanmean(np.reshape(rgrLCL, (rgrLCL.shape[0],rgrLCL.shape[1]/2,2)), axis=2)
rgrLFC=np.nanmean(np.reshape(rgrLFC, (rgrLFC.shape[0],rgrLFC.shape[1]/2,2)), axis=2)
rgrSH03=np.nanmean(np.reshape(rgrSH03, (rgrSH03.shape[0],rgrSH03.shape[1]/2,2)), axis=2)
rgrSH06=np.nanmean(np.reshape(rgrSH06, (rgrSH06.shape[0],rgrSH06.shape[1]/2,2)), axis=2)
...
np.savez('SSP_daily.npz',rgrSPP_I=rgrSPP_I, rbgrLon=rgiStatLon, rbgrLat=rgiStatLat, rgdTimeFin=rgdTimeFin)
...
rgsVars=['Snow','WBZ','ThreeC','FLH','TS','LRbF','CAPE','CIN','LCL','LFC', 'SH03','SH06','Snow_PRdays', 'Rain_PRdays']
