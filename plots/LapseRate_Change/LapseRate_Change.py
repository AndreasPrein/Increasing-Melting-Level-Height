...
RS_WCLD=RS_data[:,:,:,VARIABLES.index('LRbF')]
RS_WCLD[:,:,(np.max(RS_Breaks[:,:,VARIABLES.index('LRbF')], axis=0) > 0.5)]=np.nan# ; RS_WCLD[:,:,np.sum(~np.isnan(RS_WCLD), axis=(0,1)) < (RS_WCLD.shape[0]*RS_WCLD.shape[1]*0.8)]=np.nan

EI_WCLD=EI_data[:,:,:,:,VARIABLES.index('LRbF')]
EI_WCLD[:,:,(np.max(EI_Breaks[:,:,:,VARIABLES.index('LRbF')], axis=0) > 0.5)]=np.nan# ; EI_WCLD[:,:,np.sum(~np.isnan(EI_WCLD), axis=(0,1)) < (EI_WCLD.shape[0]*EI_WCLD.shape[1]*0.8)]=np.nan

E20_WCLD=E20_data[:,:,:,:,VARIABLES.index('LRbF')]
E20_WCLD[:,:,(np.max(E20_Breaks[:,:,:,VARIABLES.index('LRbF')], axis=0) > 0.5)]=np.nan# ; E20_WCLD[:,:,np.sum(~np.isnan(E20_WCLD), axis=(0,1)) < (E20_WCLD.shape[0]*E20_WCLD.shape[1]*0.8)]=np.nan
...
for se in range(5):
    ...
    rgrSeasonalData=np.mean(rgrSeasonalData,axis=1)
    ...
    rgrSeasonalData_EI=np.mean(rgrSeasonalData_EI,axis=1)
...
sLabel='$\Delta$ surf. to ML lapse rate'
sUnit='[$^{\circ}$C km$^{-1}$]'
rgrDataVar=E20_WCLD
...
for sr in range(len(rgsGlobal)):
    ...
    for da in range(len(rgsData)):
        ...
        try:
            plt.plot(rgiYearsAct, medfilt(rgrTMPData,kernel_size=11), c=sColor, lw=3, zorder=3, label=rgsData[da])
        except:
            stop()
...
sPlotFile=PlotDir
sPlotName= 'Surf-to-ML_lapser-rate-changes.pdf'
