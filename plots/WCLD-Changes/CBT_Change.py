...
RS_WCLD=RS_data[:,:,:,VARIABLES.index('CBT')]
RS_WCLD[:,:,(np.max(RS_Breaks[:,:,VARIABLES.index('CBT')], axis=0) > 0.5)]=np.nan

EI_WCLD=EI_data[:,:,:,:,VARIABLES.index('CBT')]
EI_WCLD[:,:,(np.max(EI_Breaks[:,:,:,VARIABLES.index('CBT')], axis=0) > 0.5)]=np.nan

E20_WCLD=E20_data[:,:,:,:,VARIABLES.index('CBT')]
E20_WCLD[:,:,(np.max(E20_Breaks[:,:,:,VARIABLES.index('CBT')], axis=0) > 0.5)]=np.nan

# RS_WCLD=RS_data[:,:,:,VARIABLES.index('LCL')]
# RS_WCLD[:,:,(np.max(RS_Breaks[:,:,VARIABLES.index('LCL')], axis=0) > 0.5)]=np.nan

# EI_WCLD=EI_data[:,:,:,:,VARIABLES.index('LCL')]
# EI_WCLD[:,:,(np.max(EI_Breaks[:,:,:,VARIABLES.index('LCL')], axis=0) > 0.5)]=np.nan

# E20_WCLD=E20_data[:,:,:,:,VARIABLES.index('LCL')]
# E20_WCLD[:,:,(np.max(E20_Breaks[:,:,:,VARIABLES.index('LCL')], axis=0) > 0.5)]=np.nan
...
for se in range(5)[:1]:
    ...
    clevsTD=np.arange(-1.4,1.4+0.0001,0.2)
