...
#             for va in range(len(rgsVars)):
#                 if rgsVars[va] in ['CAPE','CIN','LCL','LFC']:
#                     sFileFin=sSaveDataDir+'ERA-Int/Sounding/'+sDate+'_ERA-Int_Hail-Env_RDA_Sounding.nc'
#                 elif rgsVars[va] in ['SH03','SH06']:
#                     continue
#                 else:
...
#     # if rgsVars[va] in ['SH06']:
#     #     sFileFin='/glade/scratch/prein/ERA-Interim/Shear06/Shear06/fin_0-6km-Shear_ERA-Interim_12-0_'+str(rgiYY_EI[yy])+'.nc'
#     #     ncid=Dataset(sFileFin, mode='r')
#     #     rgrDataTMP=np.squeeze(ncid.variables['06Shear'][:])
#     #     ncid.close()
#     #     for mm in range(12):
#     #         iYYYY=rgdTimeDD_EI[rgdTimeDD_EI.year == rgiYY_EI[yy]]
#     #         rgiDD=(iYYYY.month == (mm+1))
#     #         rgrMonthlyData_EI[yy,mm,:,:,rgsVars.index('SH06')]=np.nanmean(rgrDataTMP[rgiDD,:,:], axis=0)
#     #     stop()
...
for va in [4]: #range(len(rgsVars[:5)):
    ...
    if rgsVars[va] == 'LCL':
        ...
        sLabel='WCL depth'
        sUnit='[km]'
        rgrDataVar=rgrMonthlyData[:,:,:,:,rgsVars.index('WBZheight')]/1000.-rgrMonthlyData[:,:,:,:,rgsVars.index('LCL')]/1000.
        rgrDataVarEI=rgrMonthlyData_EI[:,:,:,:,rgsVars.index('WBZheight')]/1000.-rgrMonthlyData_EI[:,:,:,:,rgsVars.index('LCL')]/1000.
        rgrROdata=rgrMonDat[:,:,:,rgsVariables_I.index('WBZ')]/1000.-rgrMonDat[:,:,:,rgsVariables_I.index('LCL')]/1000.
        Yrange=[-0.2,0.3]
        ...
        for sr in range(len(rgsSubregions)):
            ...
            for da in range(len(rgsData)):
                ...
                if rgsSubregions[sr] == 'Africa':
                    a = plt.axes([.57, .29, .15, .22], facecolor=None)
                if rgsSubregions[sr] == 'Asia':
                    a = plt.axes([.76, .63, .15, .22], facecolor=None)
                if rgsSubregions[sr] == 'North-America':
                    a = plt.axes([.29, .61, .15, .22], facecolor=None)
                if rgsSubregions[sr] == 'South-America':
                    a = plt.axes([.33, .10, .15, .22], facecolor=None)
                if rgsSubregions[sr] == 'Australia':
                    a = plt.axes([.81, .14, .15, .22], facecolor=None)
                if rgsSubregions[sr] == 'Europe':
                    a = plt.axes([.52, .67, .15, .22], facecolor=None)
                a.patch.set_alpha(0)
        ...
        for sr in range(len(rgsGlobal)):
            for da in range(len(rgsData)):
                ...
                if rgsGlobal[sr] == 'Global':
                    ...
                    a = plt.axes([0.05, .7, .15, .22]) #, facecolor='y')
                if rgsGlobal[sr] == 'Global Land':
                    a = plt.axes([0.05, .39, .15, .22]) #, facecolor='y')
                    flags=rgiLandOcean
                if rgsGlobal[sr] == 'Global Ocean':
                    a = plt.axes([0.05, .08, .15, .2]) #, facecolor='y')
            ...
            if sr == 0:
                ...
                a.legend(ncol=1, prop={'size':8},\
                         bbox_to_anchor=(4.2, -2.1), framealpha=0.3,fancybox=True)
